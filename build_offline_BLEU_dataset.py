...
if __name__ == "__main__":
    # Code used to generate offline datasets with noisy BLEU rewards
    # argv: seed
    argv = sys.argv[1]
    os.environ["CUDA_VISIBLE_DEVICES"] = argv
    average_rewards = []
    # generate dataset for in-domain experiment
    for remove_top in range(4):
        r = get_cfg_and_run(100 + int(argv), model="baseline_40_0.5", noise=0, remove_top=remove_top)
        average_rewards.append(r)
        sys.argv = [sys.argv[0], argv]
    # generate dataset for cross-domain experiment
    for noise in [0, 1, 3, 5]:
        r = get_cfg_and_run(100 + int(argv), model="baseline_wmt", adapt=True, noise=noise)
        average_rewards.append(r)
        sys.argv = [sys.argv[0], argv]
    print(r)
