...
def main(cfg, out_dir, src_pfx, tgt_pfx, data_key="base", num_samples=5, noise=1, remove_top=0):
    ...
    def decode(toks, is_hyp=True):
        if is_hyp:
            ...
            toks = utils.strip_pad(toks, src_dict.pad())
            s = src_dict.string(toks.int().cpu(), task.cfg.eval_bleu_remove_bpe,
    ...
    for sample in progress:
        ...
        decoded_sources = [decode(each, is_hyp=False) for each in sorted_sources]
        sources_encode = model.encode(decoded_sources, show_progress_bar=False)
        ...
        for i in range(len(hypos[0])):
            ...
            decoded_hypos = [decode(each) for each in sorted_hypo_tokens]
            hypos_encode = model.encode(decoded_hypos, show_progress_bar=False)
            all_rewards.append(np.diag(util.cos_sim(sources_encode, hypos_encode).data.numpy()) * 100)
...
def get_cfg_and_run(seed, epoch="_best", model="baseline_wmt", num_samples=5, beam_size=50, noise=0, adapt=False, remove_top=0):
    ...
    if adapt:
        OUT_DIR = f"data-bin/wmt17.iwslt.mss_{model}{epoch}_{num_samples}_{beam_size}_{noise}_{remove_top}.{seed}.en-de"
        BASE_DATA_DIR = "data-bin/wmt17.en-de.iwslt"
    else:
        OUT_DIR = f"data-bin/iwslt.mss_{model}{epoch}_{num_samples}_{beam_size}_{noise}_{remove_top}.{seed}.en-de"
...
if __name__ == "__main__":
    # Code used to generate offline datasets with rewards of semantic similarity
    # argv: seed
    argv = sys.argv[1]
    os.environ["CUDA_VISIBLE_DEVICES"] = argv
    # generate dataset for in-domain experiment
    r1 = get_cfg_and_run(100 + int(argv), model="baseline_40_0.5", noise=0, remove_top=0)
    sys.argv = [sys.argv[0], argv]
    # generate dataset for cross-domain experiment
    r1 = get_cfg_and_run(100 + int(argv), model="baseline_wmt", noise=0, remove_top=0, adapt=True)
    print(r1)
